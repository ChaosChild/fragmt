...
- **2FA write-mode quirk (hit live, 2026-08-21, release run 2):** with the npm account's 2FA set to "Authorization and writes", publishes fail with **EOTP** even when authenticated by token — CI cannot produce TOTP codes. Fix is account-side: 2FA → "Authorization only" (login stays 2FA-guarded; tokens alone authorize writes), then re-run the failed workflow — no code or tag change. Provenance statements already signed into the sigstore transparency log by a failed run are orphaned but harmless. The alternative (a classic Automation token, which bypasses 2FA) was rejected: same effective write security, but it loses the granular token's scoping and expiry.

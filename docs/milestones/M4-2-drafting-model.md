...
- **Cards** (item 1): `.dc-title` without `.md`, `.dc-ver` = `vN`, `.dc-meta` = author · date (+ `.dc-draft` status chip), `.dc-snippet` = first non-heading, non-empty body line clamped ~110 chars (fs read, not git). File actions are two small icons (rename/move, delete) in the card's top-right corner, left of the version chip – **revealed on hover/focus-within** (≥32px targets, keyboard-reachable).
...
1. Row file actions moved inside the card: the kebab rendered *beside* the card, leaving a dead column and squashing the card text. Replaced by two small icons (rename/move, delete) in the card's top-right corner, left of the version chip – folder rows get the same pair. Hover/focus reveal and ≥32px targets unchanged.
